 ...
 * 20250303 Yoshinori Kurimoto Modification
...
#define DMA_BUF_LENGTH 32767
#endif

#define ADC_CONTEXT_USES_KERNEL_TIMER
#define ADC_CONTEXT_ENABLE_ON_COMPLETE
#include "adc_custom.h"
#include "adc_context_custom.h"
			       ...
			       void *buffer, size_t channel_count)
{
	...
	LOG_DBG("start DMA configuration");
	...
	LOG_DBG("start DMA configuration2");
	LOG_DBG("dma slot %d", dma->dma_cfg.dma_slot);
	LOG_DBG("dma channel direction %d", dma->dma_cfg.channel_direction);
	...
	LOG_DBG("DMA is configured");
	adc_stm32_enable_dma_support(adc);
	LOG_DBG("DMA is enabled");
	data->dma_error = 0;
			     ...
			     uint8_t active_channels)
{
	...
	struct adc_averaging_ctx* avctx = (struct adc_averaging_ctx *)(sequence->options->user_data);
	needed_buffer_size = active_channels * sizeof(uint16_t);

	if (sequence->options) {
		needed_buffer_size *= (1 + sequence->options->extra_samplings)/avctx->num_averages;
	}
			 ...
			 uint32_t channel, int status)
{
	...
	struct adc_averaging_ctx *avctx = data->ctx.options.user_data;
	ADC_TypeDef *adc = (ADC_TypeDef *)config->base;

	LOG_DBG("dma callback");
	LOG_DBG("channel = %d, %d, status = %d", channel, data->dma.channel, status);
	...
	if (channel == data->dma.channel) {
		...
		if (status >= 0) {
#endif /* !DT_HAS_COMPAT_STATUS_OKAY(st_stm32f1_adc) */
			if ((avctx->remaining_count != 0) && (avctx->start_flag == true)){
				avctx->buf_sum[avctx->current_idx] += (uint32_t)(data->buffer[0]);
				data->samples_count = data->channel_count;
				if(avctx->current_idx == 0)
					data->buffer = data->repeat_buffer;
				else
					data->buffer += data->channel_count;
				avctx->current_idx += 1;
			}
			if (avctx->current_idx == avctx->buf_size / sizeof(uint32_t)) {
				avctx->current_idx = 0;
				avctx->remaining_count = avctx->remaining_count - 1;
				data->buffer -= avctx->buf_size / sizeof(uint32_t);
			}
			
			//data->samples_count = 32767;
			//data->buffer += 32767;
...
static int set_sequencer(const struct device *dev)
{
	...
	for (uint32_t channels = data->channels; channels;
		...
		if (config->sequencer_type == FULLY_CONFIGURABLE) {
			...
			LOG_DBG("selected channel = %d, %d", channel, channel_id);
			LOG_DBG("CH15 selection = %d",LL_ADC_GetChannelPreselection(adc, LL_ADC_CHANNEL_15));
...
static void adc_context_start_sampling(struct adc_context *ctx)
{
	...
	//adc_stm32_dma_start(dev, data->buffer, 32767);
#endif
...
static void adc_stm32_isr(const struct device *dev)
{
	...
	//LOG_DBG("%s ISR triggered. %d", dev->name, data->samples_count);
}
#endif /* !CONFIG_ADC_STM32_DMA */
...
int adc_stm32_read(const struct device *dev,
			  const struct adc_sequence *sequence)
{
	struct adc_stm32_data *data = dev->data;
	int error;
...
int adc_stm32_channel_setup(const struct device *dev,
...
#endif
	.ref_internal = STM32_ADC_VREF_MV, 
};
*/
#if defined(CONFIG_SOC_SERIES_STM32F0X)
